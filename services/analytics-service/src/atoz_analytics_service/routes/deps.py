"""FastAPI dependencies: auth, tenancy, and service resolution for routes."""
...
def get_analytics_service(request: Request) -> AnalyticsService:
    ...
    if service is None:
        raise ServiceUnavailableError("The analytics database is not configured.")
    return service
...
async def resolve_public_niche(
    ...
    """Collector identifies the niche by slug (never by id)."""
    if not niche:
