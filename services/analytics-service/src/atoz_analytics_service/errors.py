"""RFC 7807 (problem+json) error handling for analytics-service.

Mirrors the frozen gateway error model (12-api-contracts.md §6) and the
content/affiliate/pinterest service conventions so every surface of the
business layer speaks the same error language.
"""
...
logger = logging.getLogger("atoz.analytics.errors")
...
class AccountIsolationError(AppError):
    """422 — account-scoped query used without account context."""

    def __init__(
        self, detail: str = "Account context is required for account-scoped access."
    ) -> None:
        super().__init__(422, "ACCOUNT_CONTEXT_REQUIRED", detail)


class ServiceUnavailableError(AppError):
    """503 — dependency (search index, crawl API) not configured."""
...
class SearchUnavailableError(AppError):
    """503 — search index is down or not configured."""

    def __init__(self, detail: str = "Search is temporarily unavailable.") -> None:
        super().__init__(503, "SEARCH_UNAVAILABLE", detail)


class RemoteApiError(AppError):
    """502/503 — upstream crawl/search API failure with a retry decision."""

    def __init__(self, detail: str, *, retryable: bool = False) -> None:
        status_code = 503 if retryable else 502
        super().__init__(status_code, "REMOTE_API_ERROR", detail, retryable=retryable)
