"""Alembic environment for admin-service (admin_db).

Async engine so the same environment runs against PostgreSQL (asyncpg) in
production/CI and SQLite (aiosqlite) in the migration test. The admin stream
owns a distinct Alembic version table so content + affiliate + pinterest +
seo + analytics + admin streams coexist on the same physical database.
"""
...
from atoz_admin_service.domain.entities import Base
