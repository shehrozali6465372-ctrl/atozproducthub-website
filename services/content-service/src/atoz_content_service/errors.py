"""RFC 7807 (problem+json) error handling for content-service.

Mirrors the frozen gateway error model (12-api-contracts.md §6) so every
surface of the business layer speaks the same error language. Services and
routes raise :class:`AppError` subclasses; the FastAPI exception handlers
render them as problem+json with stable codes.
"""
...
logger = logging.getLogger("atoz.content.errors")
...
def register_exception_handlers(app: FastAPI) -> None:
    """Attach the RFC 7807 handlers used by every content route."""
